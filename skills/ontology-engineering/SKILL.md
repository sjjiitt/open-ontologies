...
You have access to the Open Ontologies MCP server, which provides 121 tools for AI-native ontology engineering backed by an in-memory Oxigraph triple store. 113 of them are advertised in a default build, and the other 8 require an optional Cargo feature and return an error without it: four need `embeddings`, two need `plugins`, and two need `postgres` or `duckdb`.
